...
export default function MissionImageSlider() {
          ...
          setMissionImages(
            Array.isArray(data) && data.length
              ...
              : [
                  ...
                  {
                    id: 1,
                    image: "/about-mission-placeholder.jpeg",
                    alt: "Clinic mission",
                  },
                ]
          )
  ...
  useEffect(() => {
    if (!missionImages.length) return
    const interval = setInterval(() => {
      setCurrentSlide((current) => (current + 1) % missionImages.length)
    }, 3500)
  ...
  const nextSlide = useCallback(() => {
    goToSlide((currentSlide + 1) % missionImages.length)
  }, [currentSlide, goToSlide, missionImages.length])

  const prevSlide = useCallback(() => {
    goToSlide((currentSlide - 1 + missionImages.length) % missionImages.length)
  }, [currentSlide, goToSlide, missionImages.length])

  if (!missionImages.length) {
    return null
  }

  return (
    <div className="relative h-96 w-full group animate-fade-in-up">
      {/* Slides Container */}
          ...
          {missionImages.map((_, index) => (
            <button
              ...
              className={cn(
                ...
                index === currentSlide ? "w-8 h-2.5 bg-white shadow-md" : "w-2.5 h-2.5 bg-white/60 hover:bg-white/80"
              )}
