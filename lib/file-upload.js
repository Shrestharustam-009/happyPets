...
function generateSecureFilename(originalName) {
  ...
  // Strict allowlist for safety against execution
...
// 2. TEAM IMAGES
// ==========================================
const TEAM_UPLOAD_DIR = path.join(process.cwd(), "public", "uploads", "team")
...
// ==========================================
const CONSENT_UPLOAD_DIR = path.join(process.cwd(), "public", "uploads", "consent-forms")
...
// 6. PATIENT PHOTOS
// ==========================================
const PATIENT_UPLOAD_DIR = path.join(process.cwd(), "public", "uploads", "patients")
if (!fs.existsSync(PATIENT_UPLOAD_DIR)) {
  fs.mkdirSync(PATIENT_UPLOAD_DIR, { recursive: true })
}

export async function savePatientPhoto(file) {
  if (isCloudinaryConfigured()) {
    return await uploadToCloudinary(file, "patients")
  }

  try {
    const buffer = await file.arrayBuffer()
    const filename = generateSecureFilename(file.name)
    const filepath = path.join(PATIENT_UPLOAD_DIR, filename)
    fs.writeFileSync(filepath, Buffer.from(buffer))
    return `/api/uploads/patients/${filename}`
  } catch (error) {
    console.error("[v0] Error saving patient photo:", error)
    throw new Error("Failed to save patient photo")
  }
}

// ==========================================
// 7. MEDICAL RECORD ATTACHMENTS (CONSENT FORMS & IMAGES)
// ==========================================
const MEDICAL_UPLOAD_DIR = path.join(process.cwd(), "public", "uploads", "medical-records")
if (!fs.existsSync(MEDICAL_UPLOAD_DIR)) {
  fs.mkdirSync(MEDICAL_UPLOAD_DIR, { recursive: true })
}

export async function saveMedicalRecordAttachment(file) {
  if (isCloudinaryConfigured()) {
    return await uploadToCloudinary(file, "medical-records")
  }

  try {
    const buffer = await file.arrayBuffer()
    const filename = generateSecureFilename(file.name)
    const filepath = path.join(MEDICAL_UPLOAD_DIR, filename)
    fs.writeFileSync(filepath, Buffer.from(buffer))
    return `/api/uploads/medical-records/${filename}`
  } catch (error) {
    console.error("[v0] Error saving medical record attachment:", error)
    throw new Error("Failed to save attachment")
  }
}